...
method_dcl: FUNC (ID|AT_ID) LPAREN param_lst RPAREN (COLON typ)? block ;
constructor_decl: FUNC CONSTRUCTOR LPAREN param_lst RPAREN block ;
param_lst: param (COMMA param)*|;
param: (iden | idlst) COLON (typ |array_type);
...
return_state: RETURN expr? SEMICOLON ;//return ;
break_state: BREAK SEMICOLON ;
callstmt: (instance_method_invo_access | static_method_invo_access | io_st) SEMICOLON;
instance_method_invo_access:  expr8 (LBRACK expr RBRACK)? DOT ID (LPAREN expr_lst RPAREN)  ;  
...
fieldaccess : expr8 (LBRACK expr RBRACK)? DOT ID | (ID DOT)? AT_ID;
...
expr8: expr8 (LBRACK expr RBRACK)? DOT ID (LPAREN expr_lst RPAREN)? |  expr9; //instance access operator
...
expr11: LPAREN expr RPAREN | literal | ID | NULL| SELF;
...
/*TYPE */
typ: INT| FLOAT| BOOL |STRING | ID | VOID;
